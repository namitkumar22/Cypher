...
def DefaultCamera():
    ...
    while True:
        ...
        if (hour == 11 and minute >= 45) or (hour == 12 and minute <= 40):
            print("Lunch Break")
            time.sleep(3300)
        elif hour >= 17:
...
class CypherGUIAdmin(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.sidebar_button_4 = customtkinter.CTkButton(self.sidebar_frame, text="Add Camera", command=self.AddCamera)
        ...
        self.input_IP_dict = self.load_camera_info()
        self.button_counter = len(self.input_IP_dict)
        for camera_name, camera_ip in self.input_IP_dict.items():
            button = customtkinter.CTkButton(self.sidebar_frame2, text=camera_name, command=partial(self.camera_task, camera_name))
            button.pack(pady=5)
    ...
    def save_camera_info(self):
        with open("camera_info.json", "w") as file:
            json.dump(self.input_IP_dict, file)

    def camera_task(self, camera_name):
        print(f"Task for {camera_name}")
        camera_ip = self.input_IP_dict.get(camera_name)
        ShowInfo(camera_ip)
            

    
    def add_button(self, camera_name, camera_ip):
        button = customtkinter.CTkButton(self.sidebar_frame2, text=camera_name, command=partial(self.camera_task, camera_name))
        button.pack(padx=10, pady=10)
        self.input_IP_dict[camera_name] = camera_ip
    ...
    def AddCamera(self):
        addcameraRoot = customtkinter.CTk()

        customtkinter.set_appearance_mode("dark")
        customtkinter.set_default_color_theme("green")
        addcameraRoot.title("Cypher 3.0")

        window_width = 400
        window_height = 300
        addcameraRoot.geometry(f"{window_width}x{window_height}")

        label_camera_name = customtkinter.CTkLabel(master=addcameraRoot, text="Camera Name")
        label_camera_name.pack(pady=5)
        self.input_camera_name = customtkinter.CTkEntry(master=addcameraRoot, width=300, height=35)
        self.input_camera_name.pack(pady=5)

        label_IP = customtkinter.CTkLabel(master=addcameraRoot, text="IP")
        label_IP.pack(pady=5)
        self.input_IP = customtkinter.CTkEntry(master=addcameraRoot, width=300, height=35)
        self.input_IP.pack(pady=5)

        buttonAdd = customtkinter.CTkButton(addcameraRoot, text="Add Camera", command=self.handle_add_camera)
        buttonAdd.pack(pady=10)
        buttonback = customtkinter.CTkButton(addcameraRoot, text="Back", command=addcameraRoot.destroy)
        buttonback.pack(pady=10)

        addcameraRoot.mainloop()

    def handle_add_camera(self):
        camera_name = self.input_camera_name.get()
        camera_ip = self.input_IP.get()
        if camera_name and camera_ip:
            self.add_button(camera_name, camera_ip)
